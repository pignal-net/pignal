...
| Emoji icons (💻🌐📌) | Quick placeholder that never gets replaced | Use SVG icons from `web/src/components/icons.tsx` |
| Creating a `styles.css` per template | Old Pico CSS pattern | All templates set `styles: ''`. Use Tailwind utilities directly in JSX |
