...
- `db/src/schema.ts` — Drizzle table definitions (`signals`, `signalTypes`, `typeActions`, `workspaces`, `settings`, `apiKeys`). Drizzle infers `SignalSelect`, `SignalInsert`, etc.
- `db/src/types.ts` — Hand-written interfaces:
  - `SignalStoreRpc` — 57-method interface defining all business operations
  - `SignalWithMeta` — Signal joined with type name, workspace name, validation action label, parsed tags array
...
- `core/src/store/signal-store.ts` — `class SignalStore implements SignalStoreRpc` (accepts any Drizzle SQLite DB)
- `core/src/routes/*.ts` — Route factories accept `RouteFactoryConfig { getStore, middleware? }`, call `config.getStore(c)` to get `SignalStoreRpc`
- `core/src/validation/schemas.ts` — Zod schemas (independent of Drizzle types, but field names must match)
- `core/src/mcp/tools.ts` — Tool functions accept `SignalStoreRpc` + validated params
- `core/src/types.ts` — `RouteFactoryConfig`, `PUBLIC_SIGNAL_FIELDS`

### `@pignal/server` — Wires Everything

- `server/src/middleware/store.ts` — Creates `SignalStore` from `env.DB` (D1), sets on Hono context as `store`
