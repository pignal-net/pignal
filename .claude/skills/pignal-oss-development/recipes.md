...
## 1. Add a New Signal Field

1. **Schema** — Add column to `signals` table in `db/src/schema.ts`
2. **Types** — Update `SignalWithMeta` and relevant interfaces in `db/src/types.ts`. If the field affects store methods, update `SignalStoreRpc`
3. **Store** — Update queries in `core/src/store/signal-store.ts` (select, insert, update as needed)
4. **Validation** — Add to Zod schemas in `core/src/validation/schemas.ts` (e.g., `createSignalSchema`, `updateSignalSchema`)
5. **MCP** — If exposed to AI clients, update tool schemas and `formatSignal()` in `core/src/mcp/tools.ts`, then update manifest in `core/src/mcp/manifest.ts`
6. **Migration** — Create `server/migrations/NNNN_description.sql` with `ALTER TABLE signals ADD COLUMN ...`
7. **Verify** — `pnpm type-check` then `pnpm db:migrate`

## 2. Add a New API Endpoint

1. **Route factory** — Add handler in the appropriate `core/src/routes/*.ts` file (signals, types, workspaces, stats, settings, or public). Uses `config.getStore(c)` to get the store
