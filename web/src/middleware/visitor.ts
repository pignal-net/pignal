...
/**
 * Visitor authentication middleware for managed sites.
 *
 * Independently verifies the per-site JWT from X-Pignal-Visitor-Token header
 * using the site's own VISITOR_SITE_SECRET (set during provisioning).
 *
 * Does NOT blindly trust headers — cryptographically verifies the JWT.
 * Role is already determined by dispatch and baked into the JWT.
 *
...
// ---------------------------------------------------------------------------
// Inline HS256 JWT verification (Web Crypto API only — no external deps)
// ---------------------------------------------------------------------------
