...
(function () {
  ...
  var sourceMain = document.getElementById('source-main');
  ...
  function showSourceLoader() {
    if (sourceLoading && sourceMain) {
      sourceLoading.removeAttribute('hidden');
      sourceMain.hidden = true;
    }
  }
  ...
  function hideAll() {
    ...
    if (sourceLoading) sourceLoading.setAttribute('hidden', '');
    if (sourceMain) sourceMain.hidden = false;
  }
  ...
  document.addEventListener('click', function (e) {
    ...
    // Filter/sort/pagination links on source page — show local spinner
    if (sourceMain && link.closest('.source-page')) {
      showSourceLoader();
      return;
    }
